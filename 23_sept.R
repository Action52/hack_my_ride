...
ggmap(lx_map, extent = "device") +
  geom_path(data = shapes, aes(shape_pt_lon, shape_pt_lat, group = shape_id), size = .1, alpha = .5, color='red') +
...
stops_freq = 
  ...
  summarize(count=n()) %>%
  filter(count>=1200) # filter out least used stops

ggmap(lx_map, extent = "device") +
  geom_point(data = stops_freq,aes(x=stop_lon, y=stop_lat, size=count, fill=count), shape=21, alpha=0.8, colour = "cadetblue3")+ #plot stops with blue color
